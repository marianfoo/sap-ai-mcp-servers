...
Use the commit date (or the intended commit date) for a new item. Preserve the original date when an existing item is moved, renamed, recategorized, or duplicated into another package view. Monorepo sub-packages use the date that the individual sub-package was added.
